...
2. Orchestrator  → LLM-driven agent that picks the right executor branch
                   (with a deterministic fallback for safety)
...
5. Error recovery → integrated Error Analyzer and Fix Generator

Day 3 changes from Day 2:
- The deterministic Router node is replaced by an Orchestrator Agent.
- The Orchestrator reasons over both the Planner's metadata AND the raw
  user requirement, and can override the Planner when the requirement
  clearly contradicts the classification.
- The graph topology is UNCHANGED — same edges, same conditional routing,
  same executor loop. Only the decision node became smarter.
"""
...
# Maps Orchestrator output strings to actual node names in the graph.
...
def route_to_executor(state: AgentState) -> str:
    """
    Conditional edge from Orchestrator → which executor handles this task.

    The Orchestrator has already populated state["route"]. We translate that
    into a node name. If the route is missing or unknown, fall back to the
    simple executor (safest default — it handles the broadest range).
    """
...
def should_continue(state: AgentState) -> str:
    ...
    Why "same executor": once the Orchestrator has classified a project,
    all subsequent steps stay in the same specialized executor. We don't
    re-route mid-execution. The executor to loop back to is read from
    state["route"].
    """
    # Terminal: all done
...
def create_workflow() -> "StateGraph":
    ...
    Final architecture (Day 3 — Orchestrator Agent):

        Planner → Orchestrator ─┬─→ SimpleExecutor   ─┐
    ...
    print("[+] Added nodes: planner, orchestrator, simple_executor, "
    ...
    # Planner → Orchestrator (Orchestrator reads metadata + raw requirement)
    workflow.add_edge(NODE_PLANNER, NODE_ORCHESTRATOR)

    # Orchestrator → one of three executors (conditional based on route value)
    ...
    print("  - Planner -> Orchestrator")
    print("  - Orchestrator -> SimpleExecutor | CompiledExecutor | WebExecutor")
    ...
    print("  Orchestrator:  LLM agent that reads metadata + raw requirement,")
    print("                 picks the right executor, can override the planner")
    print("                 Routes: simple | compiled | web")
    print("                 Falls back to deterministic rules on LLM failure")
    print()
...
if __name__ == "__main__":
    ...
    print(f"  Route:              {mock_state.get('route')} (set by Orchestrator)")

    print("\nExpected flow for this requirement:")
    print("  1. Planner classifies as 'script', sets project_type='script'")
    print("  2. Orchestrator reasons over metadata + requirement, sets route='simple'")
